...
router.get('/api/public/viral-videos', async (req, res) => {
  try {
    let videosList;
...
router.post('/api/public/viral-videos/:id/interact', async (req, res) => {
  try {
    ...
    // Verify authentication (reuse the same logic or middleware)
    // We can use the middleware logic embedded here or call it
    // For simplicity, let's look up the user first

    // ... Copying authentication logic from news interaction ...
    let userInfo = null;
    try {
      ...
      // Extract detailed user info similar to news interaction
      if (user.googleId) {
    ...
    if (isConnectedToMongoDB) {
      ...
      // Update User (lastLogin etc)
      const User = require('../models/User');
      let user = await User.findById(userId); // Already fetched above, but strictly logic
      user.lastLogin = new Date();
      await user.save();

      // Handle interactions on video (similar logic to News.js)
      if (!video.userInteractions) video.userInteractions = { likes: [], dislikes: [], comments: [] };
      ...
      // Return updated video object (transformed)
...
router.post('/api/public/news/:id/report', async (req, res) => {
  try {
    const { id } = req.params;
    let { reason, description, userId, userEmail, userName } = req.body;
